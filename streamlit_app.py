...
with tab1:
    ...
    with button_col1:
        if st.button("✅ Select All", use_container_width=True):
            st.session_state.select_all_langs = True
            st.session_state.deselect_all_langs = False
    
    with button_col2:
        if st.button("❌ Deselect All", use_container_width=True):
            st.session_state.deselect_all_langs = True
            st.session_state.select_all_langs = False
    
    # Initialize session state for bulk actions
    if 'select_all_langs' not in st.session_state:
        st.session_state.select_all_langs = False
    if 'deselect_all_langs' not in st.session_state:
        st.session_state.deselect_all_langs = False
    
    # Determine default value based on bulk actions
    default_value = True
    if st.session_state.select_all_langs:
        default_value = True
    elif st.session_state.deselect_all_langs:
        default_value = False
    ...
    with col1:
        lang_selections['EN'] = st.checkbox('English', value=default_value, key='lang_en')
        lang_selections['DE'] = st.checkbox('German', value=default_value, key='lang_de')
        lang_selections['FR'] = st.checkbox('French', value=default_value, key='lang_fr')
        lang_selections['ES'] = st.checkbox('Spanish', value=default_value, key='lang_es')
    
    with col2:
        lang_selections['PT'] = st.checkbox('Portuguese', value=default_value, key='lang_pt')
        lang_selections['IT'] = st.checkbox('Italian', value=default_value, key='lang_it')
        lang_selections['CZ'] = st.checkbox('Czech', value=default_value, key='lang_cz')
        lang_selections['PL'] = st.checkbox('Polish', value=default_value, key='lang_pl')
    
    with col3:
        lang_selections['TK'] = st.checkbox('Turkish', value=default_value, key='lang_tk')
        lang_selections['VI'] = st.checkbox('Vietnamese', value=default_value, key='lang_vi')
        lang_selections['TH'] = st.checkbox('Thai', value=default_value, key='lang_th')
        lang_selections['ID'] = st.checkbox('Indonesian', value=default_value, key='lang_id')
    
    with col4:
        lang_selections['KO'] = st.checkbox('Korean', value=default_value, key='lang_ko')
        lang_selections['CN'] = st.checkbox('Chinese (CN)', value=default_value, key='lang_cn')
        lang_selections['TW'] = st.checkbox('Chinese (TW)', value=default_value, key='lang_tw')
        lang_selections['MX'] = st.checkbox('Spanish (MX)', value=default_value, key='lang_mx')
    
    # Reset bulk action flags after rendering
    if st.session_state.select_all_langs:
        st.session_state.select_all_langs = False
    if st.session_state.deselect_all_langs:
        st.session_state.deselect_all_langs = False
