...
# Initialize target language selections
if 'target_selections' not in st.session_state:
    st.session_state.target_selections = {code: True for code in LANGUAGES.keys() if code != 'ja'}
...
with st.sidebar:
    ...
    grok_key = st.text_input("Grok API Key (Primary)", type="password", 
                              help="Required: Grok 4 Fast with 2M context")
    gemini_key = st.text_input("Gemini API Key (Backup)", type="password",
    ...
    **System Capabilities:**
    ...
    with tab1:
        ...
        with col1:
            ...
            if source_lang != st.session_state.source_lang:
                ...
                st.session_state.target_selections = {
                    code: True for code in LANGUAGES.keys() if code != source_lang
                }
                st.rerun()
        ...
        with btn_col1:
            if st.button("✅ Select All", use_container_width=True):
                for code in LANGUAGES.keys():
                    if code != source_lang:
                        st.session_state.target_selections[code] = True
                st.rerun()
        
        with btn_col2:
            if st.button("❌ Deselect All", use_container_width=True):
                for code in LANGUAGES.keys():
                    st.session_state.target_selections[code] = False
                st.rerun()
        ...
        for i, (code, name) in enumerate(LANGUAGES.items()):
            if code != source_lang:  # Exclude source language
                with cols[i % 4]:
                    st.session_state.target_selections[code] = st.checkbox(
                        name,
                        value=st.session_state.target_selections.get(code, True),
                        key=f'target_{code}'
                    )
        
        # Show selected count
        selected_targets = [code for code, selected in st.session_state.target_selections.items() 
                           if selected and code != source_lang]
    ...
    with tab2:
        ...
        if uploaded_files:
            ...
            # Language checkboxes
            cols = st.columns(4)
            file_target_selections = {}
            
            for i, (code, name) in enumerate(LANGUAGES.items()):
                if code != file_source_lang:
                    with cols[i % 4]:
                        default_value = st.session_state.get(f'file_target_{code}', True)
                        file_target_selections[code] = st.checkbox(
                            name,
                            value=default_value,
                            key=f'file_target_{code}'
                        )
            
            selected_file_targets = [code for code, selected in file_target_selections.items() if selected]
